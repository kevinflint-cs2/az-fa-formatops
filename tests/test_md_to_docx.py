"""
Unit tests for md_to_docx module.
...
from functions import md_to_docx
...
def test_handle_request_success():
    ...
    with patch("functions.md_to_docx._markdown_to_docx_base64") as mock_convert:
...
def test_handle_request_missing_markdown():
    ...
    with pytest.raises(ValueError, match="missing 'markdown' parameter"):
        md_to_docx.handle_request(payload)
...
def test_markdown_to_docx_base64_success():
    ...
    with (
        ...
        assert decoded == b"DOCX_DATA"


@pytest.mark.mock
def test_markdown_to_docx_base64_pandoc_error():
    markdown = "# Title"
    ...
    with (
        ...
        with pytest.raises(RuntimeError, match="Pandoc missing"):
            md_to_docx._markdown_to_docx_base64(markdown)
